  ...
  for(face = start; face < stop; face++)
    {
      ...
      flux[IVX]   = -(sts_xx * nx + sts_xy * ny + sts_xz * nz);
      flux[IVY]   = -(sts_xy * nx + sts_yy * ny + sts_yz * nz);
      flux[IVZ]   = -(sts_xz * nx + sts_yz * ny + sts_zz * nz);

      if (ftype != 3)
	{         
	  psd_flux[p0][IVX] += flux[IVX];         
	  psd_flux[p0][IVY] += flux[IVY];
	  psd_flux[p0][IVZ] += flux[IVZ];
	}
      if (ftype != 2)
	{                 
	  psd_flux[p1][IVX] -= flux[IVX];
	  psd_flux[p1][IVZ] -= flux[IVZ];
	  psd_flux[p1][IVY] -= flux[IVY];
	}
    }
}
